...
CONFIG_CLIENTID=$(sed -n '/clientid:/ s/^[ ]*clientid:[ ]*"\([^"]*\)".*/\1/p' <<< "$CONFIG_YAML")
CONFIG_CLIENTSECRET=$(sed -n '/clientsecret:/ s/^[ ]*clientsecret:[ ]*"\([^"]*\)".*/\1/p' <<< "$CONFIG_YAML")
...
if [ -e $TEMP_DIR/backup.tar.gz ]; then
  ...
  # 还原面板配置的最新信息
  sed -i "s@^realipheader:.*@$CONFIG_REALIP@; s@^language:.*@$CONFIG_LANGUAGE@; s@clientid:.*@clientid: \"$CONFIG_CLIENTID\"@; s@clientsecret:.*@clientsecret: \"$CONFIG_CLIENTSECRET\"@" ${TEMP_DIR}/${FILE_PATH}data/config.yaml
  ...
  if [ "$IS_DOCKER" = 1 ]; then
    ...
    [ "$DB_TOKEN" != "$LOCAL_TOKEN" ] && ID=$(awk -F ': ' '/^uuid:/ {print $2}' /dashboard/agent/agent.yml) && sqlite3 ${TEMP_DIR}/${FILE_PATH}data/sqlite.db "update servers set uuid='${ID}' where created_at='2024-12-11 15:19:41.6189758+08:00'" && sed -i "s/^client_secret: .*/client_secret: ${DB_TOKEN}/" "/dashboard/agent/agent.yml"
  fi
